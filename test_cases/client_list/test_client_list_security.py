...
class TestSecuritySnapshotPrice:
    ...
    def test_snapshot_price_default_issue_type(self, client_list_api):
        """
        测试场景2：不传 issue_type 使用默认值（Common Stock）
        Test Scenario2: Snapshot Price with Default issue_type (Common Stock)
        验证点：
        1. HTTP 200，code=200
    ...
    def test_snapshot_price_all_issue_types(self, client_list_api, issue_type):
        """
        测试场景3：遍历所有 issue_type 枚举值
        Test Scenario3: Get Snapshot Price for All issue_type Enum Values
        验证点：
        1. HTTP 200
    ...
    def test_snapshot_price_missing_symbol(self, client_list_api):
        """
        测试场景4：缺少必填参数 symbol
        Test Scenario4: Missing Required Symbol Returns Business Error
        验证点：
        1. HTTP 200，code=599
    ...
    def test_snapshot_price_invalid_symbol(self, client_list_api):
        """
        测试场景5：使用不存在的证券代码
        Test Scenario5: Non-existent Symbol Returns Empty or Error
        """
        resp = client_list_api.get_security_snapshot_price(
    ...
    def test_snapshot_price_invalid_issue_type(self, client_list_api):
        """
        测试场景6：使用文档未定义的 issue_type
        Test Scenario6: Invalid issue_type Returns Error or Falls Back
        """
        resp = client_list_api.get_security_snapshot_price(
    ...
    def test_snapshot_price_different_symbols(self, client_list_api):
        """
        测试场景7：测试多个常见证券代码
        Test Scenario7: Get Snapshot Price for Multiple Common Symbols
        验证点：每个 symbol 均能正常查询（不报错）
        """
        symbols = ["AAPL", "MSFT", "GOOGL", "VT"]
        for sym in symbols:
...
class TestSecurityHistoricalChart:
    ...
    def test_historical_chart_all_intervals(self, client_list_api, interval):
        """
        测试场景2：遍历所有 interval 枚举值（1D/5D/1M/3M/1Y/ALL）
        Test Scenario2: Historical Chart for All interval Enum Values
        验证点：
        1. HTTP 200，code=200
    ...
    @pytest.mark.parametrize("issue_type", ["Common Stock", "ETF", "Mutual Funds"])
    def test_historical_chart_issue_type_coverage(self, client_list_api, issue_type):
        """
        测试场景3：常见 issue_type 枚举值覆盖
        Test Scenario3: Historical Chart for Common issue_type Values
        验证点：各 issue_type 均能正常调用，不报接口错误
        """
        resp = client_list_api.get_security_historical_chart(
    ...
    def test_historical_chart_missing_symbol(self, client_list_api):
        """
        测试场景4：缺少必填参数 symbol
        Test Scenario4: Missing Symbol Returns Business Error
        """
    ...
    def test_historical_chart_invalid_interval(self, client_list_api):
        """
        测试场景5：无效的 interval 枚举值
        Test Scenario5: Invalid interval Value Returns Error
        """
        resp = client_list_api.get_security_historical_chart(
    ...
    def test_historical_chart_response_structure(self, client_list_api):
        """
        测试场景6：验证响应结构完整性
        Test Scenario6: Verify Historical Chart Response Structure
        """
        resp = client_list_api.get_security_historical_chart(
            symbol="AAPL", interval="5D"
        )
        assert_status_ok(resp)
        body = resp.json()
        assert body.get("code") == 200
