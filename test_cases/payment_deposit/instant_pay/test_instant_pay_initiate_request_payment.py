...
已验证账户数据：
  IP_FA=251119084741475550，IP_SUB=251119084741475584
...
class TestInstantPayRFP:
    ...
    def _create_rfp(self, instant_pay_api, suffix=""):
        """创建一个 RFP 并返回 id"""
        ...
        assert resp.json().get("code") == 200, \
            f"RFP 创建失败: {resp.json().get('error_message')}"
        data = resp.json().get("data") or resp.json()
        return data.get("id")

    def test_rfp_success_basic(self, instant_pay_api):
        """
    ...
    def test_rfp_with_optional_params(self, instant_pay_api):
        """
    ...
    def test_rfp_cancel_success(self, instant_pay_api):
        ...
        assert body.get("data") is True or body.get("data") == "True" or body.get("data"), \
            f"cancel 响应 data 应为 True，实际: {body.get('data')}"
        logger.info(f"✓ RFP cancel 成功: id={rfp_id}, data={body.get('data')}")

    def test_rfp_cancel_invalid_id(self, instant_pay_api):
        """
    ...
    def test_rfp_missing_sub_account_id(self, instant_pay_api):
        """
    ...
    def test_rfp_invisible_fa(self, instant_pay_api):
        """
        测试场景6：越权 FA → 被拒绝
    ...
    def test_rfp_appears_in_list(self, instant_pay_api):
        """
        测试场景7：发起 RFP 后在 RFP list 中可查到
        Test Scenario7: Initiated RFP Appears in RFP Transactions List
        """
        rfp_id = self._create_rfp(instant_pay_api, "ListCheck")
