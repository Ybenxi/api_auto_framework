...
class TestContactList:
    ...
    def test_list_contacts_with_status_filter(self, login_session):
        ...
        # 2. 使用 status 筛选
        logger.info("使用 status='Active' 筛选 Contacts")
        ...
        if len(contacts) > 0:
            logger.info(f"  第一个 Contact: {contacts[0].get('name')} ({contacts[0].get('status')})")
    ...
    def test_list_contacts_with_name_filter(self, login_session):
        ...
        # 2. 使用 name 筛选
        logger.info("使用 name 筛选 Contacts")
    ...
    def test_list_contacts_with_email_filter(self, login_session):
        ...
        # 2. 使用 email 筛选
        logger.info("使用 email 筛选 Contacts")
    ...
    def test_list_contacts_response_fields(self, login_session):
        ...
        # 5. 验证字段完整性
        if len(contacts) > 0:
            logger.info("验证 Contact 对象字段完整性")
            contact = contacts[0]
    ...
    def test_list_contacts_sorting_by_name(self, login_session):
        ...
        if len(contacts) >= 2:
            ...
            logger.info("✓ 排序验证成功（升序），共 {len(contacts)} 个 Contacts")
            logger.info(f"  第一个: {contacts[0].get('name')}")
            logger.info(f"  最后一个: {contacts[-1].get('name')}")
        else:
